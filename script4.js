...
function initialize_lyric(rapLyric){
    $lyric1 = $("<p>Sam keene has a long ponytail</p>");
    $lyric2 = $("<p>Started from the bottom now we're here</p>");
    $lyric = $("random number thing doesnt work");
    var num_lyrics = 2;
    //console.log(lyric2);
    // BAD way to do this, but for testing purposes it works
    var rn = Math.floor(Math.random()*(num_lyrics));
    console.log (rn);
    console.log (rapLyric.attr("id"));

    if (rn == 0) {
        rapLyric.html($lyric1);
        console.log(rapLyric);
        rapLyric.attr("id","computer");
        console.log(rn);
    }
    else if (rn == 1){
        rapLyric.html($lyric2);
        console.log(rapLyric);
        rapLyric.attr("id","rap");
    }
    else {
        rapLyric.html($lyric);
        rapLyric.attr("id","computer");
